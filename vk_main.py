...
class VKBot:
    ...
    def age(self, user_id): #getting unit age
        ...
        try:
            ...
            self.write_msg(user_id, 'Введите допустимый возраст от и до (годы): ')
...
for event in bot.longpoll.listen():
    if event.type == VkEventType.MESSAGE_NEW and event.to_me:
        ...
        if request == "начать поиск":
            ...
            bot.write_msg(event.user_id, f'Нашёл для тебя пару, введи слово "да"')
            ...
            bot.write_msg(user_id, "Не понимаю")